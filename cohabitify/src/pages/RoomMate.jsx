...
const RoomMate = () => {
  ...
  const isOwner = !!(room && user && (
    ...
    (room.owner && room.owner._id && room.owner._id === user.id)
  ));
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {room.members.map(m => (
                <div key={(m.id || m._id)} className="p-3 border rounded flex items-center justify-between">
                  <div>
                    <div className="font-medium text-gray-900">{m.username}</div>
                    <div className="text-xs text-gray-600">{m.email}</div>
                  </div>
                  {room.owner?.id === m.id || (room.owner && room.owner._id === m._id) ? (
